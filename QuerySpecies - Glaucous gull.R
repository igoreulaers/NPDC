...
# Simple format -----------------------------------------------------------
LarHyp <- fieldLab_df[fieldLab_df$scientificName == "Larus hyperboreus",]
LarHyp <- LarHyp[complete.cases(LarHyp[ , "eventID"]), ]
...
LarHyp_long_simple <- LarHyp %>% select(8, 13,
                                    6, 10,
                                    19, 46, 54,
                                    56, 45, 47) %>% rename_with(~ newCOLname_simple, all_of(oldCOLname_simple))

write.xlsx(LarHyp_long_simple, file = "LarHyp - All data - simple.xlsx")
...
newCOLname_extF <- c("ID.field", "species",
                     ...
                     "matrix", "unit", "EOM.%",
                     "group", "compound", "concentration")
LarHyp_long_extF <- LarHyp %>% select(8, 13,
                                  6, 10, 4, 5, 
                                  15, 18, 17, 27,
                                  12, 21, 16,
                                  19, 46, 54,
...
write.xlsx(LarHyp_long_extF, file = "LarHyp - All data - extended field.xlsx")
...
newCOLname_extL <- c("ID.field", "species",
                     ...
                     "LOD", "LOQ", "recovery")
LarHyp_long_extL <- LarHyp %>% select(8, 13,
                                  6, 10,
                                  19, 46, 54,
                                  56, 45, 47,
                                  49, 41, 58, 42,
...
write.xlsx(LarHyp_long_extL, file = "LarHyp - All data - extended lab.xlsx")
...
newCOLname_extFL <- c("ID.field", "species",
                      ...
                      "rightsholder_lab", "lab", "responsible_lab", "date_lab",
                      "LOD", "LOQ", "recovery")
LarHyp_long_extFL <- LarHyp %>% select(8, 13,
                                   6, 10, 4, 5, 
                                   15, 18, 17, 27,
                                   12, 21, 16,
                                   19, 46, 54,
...
write.xlsx(LarHyp_long_extFL, file = "LarHyp - All data - extended field and lab.xlsx")
