...
# Simple format -----------------------------------------------------------
PagEbu <- fieldLab_df[fieldLab_df$scientificName == "Pagophila eburnea",]
PagEbu <- PagEbu[complete.cases(PagEbu[ , "eventID"]), ]
...
PagEbu_long_simple <- PagEbu %>% select(8, 13,
                                        6, 10,
                                        19, 46, 54,
                                        56, 45, 47) %>% rename_with(~ newCOLname_simple, all_of(oldCOLname_simple))

write.xlsx(PagEbu_long_simple, file = "PagEbu - All data - simple.xlsx")
...
newCOLname_extF <- c("ID.field", "species",
                     ...
                     "matrix", "unit", "EOM.%",
                     "group", "compound", "concentration")
PagEbu_long_extF <- PagEbu %>% select(8, 13,
                                      6, 10, 4, 5, 
                                      15, 18, 17, 27,
                                      12, 21, 16,
                                      19, 46, 54,
...
write.xlsx(PagEbu_long_extF, file = "PagEbu - All data - extended field.xlsx")
...
newCOLname_extL <- c("ID.field", "species",
                     ...
                     "LOD", "LOQ", "recovery")
PagEbu_long_extL <- PagEbu %>% select(8, 13,
                                      6, 10,
                                      19, 46, 54,
                                      56, 45, 47,
                                      49, 41, 58, 42,
...
write.xlsx(PagEbu_long_extL, file = "PagEbu - All data - extended lab.xlsx")
...
newCOLname_extFL <- c("ID.field", "species",
                      ...
                      "rightsholder_lab", "lab", "responsible_lab", "date_lab",
                      "LOD", "LOQ", "recovery")
PagEbu_long_extFL <- PagEbu %>% select(8, 13,
                                       6, 10, 4, 5, 
                                       15, 18, 17, 27,
                                       12, 21, 16,
                                       19, 46, 54,
...
write.xlsx(PagEbu_long_extFL, file = "PagEbu - All data - extended field and lab.xlsx")
